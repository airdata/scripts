#!/bin/bash
ARNS=$(aws rds describe-db-instances --region eu-central-1 --query "DBInstances[].DBInstanceArn" --output text)
for line in $ARNS; do
    TAGS=$(aws rds list-tags-for-resource --region eu-central-1 --resource-name "$line" --query "TagList[]")
